...
export function injectState(artifact: CompiledContractArtifact, state: DecodedState): Uint8Array {
  ...
  const slot = encodeState(state.owner, state.identifierType, state.amount, state.isMinter, state.used, state.salePrice);
  if (slot.length !== len) {
...
export function availableTicketAddress(
  ...
  return addressFor(
    artifact,
    { owner: new Uint8Array(HASH_LENGTH), identifierType: 0, amount: capacity, isMinter: false, used: false, salePrice: 0 },
    network,
    options,
  );
}
